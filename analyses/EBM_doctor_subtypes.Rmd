...
```{r}
glm(diagnosed ~ x + severity, data = rbind(df_A, df_B), family = "binomial")
```
...
simulate_EBM_docs_vary <- function(years = 10, evidence_rate = 0.1){
  for (i in 1:years){
    ...
    if (i == 1){
      ...
      curr_df_A <- diagnose_cross_sectional(curr_df_A, theta = theta_A) %>%
        ...
        select(x, disease, severity, doctor_id, doctor_type, study_id, p_model, p_doc, p_diagnose, diagnosed)
      ...
      curr_df_A <- curr_df_A %>%
        ...
        mutate(p_doc = diagnosis_fn(severity, x, theta = theta_A)) %>%
      ...
      curr_df_B <- curr_df_B %>% 
        ...
        mutate(p_doc = diagnosis_fn(severity, x, theta = theta_B)) %>%
        mutate(p_diagnose = (1 - evidence_weight)*p_doc + evidence_weight*p_model) %>%
  ...
  curr_model <- glm(diagnosed ~ x + severity, data = rbind(curr_df_A, curr_df_B), family = "binomial")
  }
