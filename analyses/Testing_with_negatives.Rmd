...
ggplot(df, aes(x = representativeness, fill = disease, group = disease)) +
...
A <- ggplot(df, aes(x = representativeness, fill = disease, group = disease)) +
  geom_density(alpha = 0.5) + 
  scale_fill_brewer(palette = "Set2") +
...
B <- ggplot(df_tested, aes(x = representativeness, fill = diagnosed, group = diagnosed)) +
  geom_density(alpha = 0.5) + 
...
df_tested %>% mutate(status = case_when(disease == FALSE ~ "No disease",
                                        ...
                                        TRUE ~ NA_character_)) %>%
ggplot(aes(x = representativeness, fill = status, group = status)) +
  geom_histogram(alpha = 0.5, position = "identity") + 
  scale_fill_brewer(palette = "Set2") +
  theme(legend.position = "bottom")  +
  xlab("Representativeness")
...
plt_data <- expand.grid(R, c(0,1)) %>%
  ...
  mutate(phi = diagnosis_fn(representativeness = R, x = X, theta = c(-10, 20, -3))) %>%
...
Now what does the researcher learn about incidence and presentation in these groups?  The incidence and distribution of representativeness is shown below.
...
df_tested %>% 
  filter(diagnosed == T) %>%
  group_by(x) %>%
  summarize(`Mean representativeness of diagnosed cases` = mean(representativeness)) %>%
  kable()
```
...
ggplot(df, aes(x = representativeness, fill = status, group = status)) +
  geom_density(alpha = 0.5) + 
  scale_fill_brewer(palette = "Set2") +
  xlab("Representativeness")
```
...
The plots below show the estimated and actual incidence (A) and the estimated and actual disease representativeness.  These visuals show a similar story to what we have seen before.
...
```{r fig.height=3, fig.width=8}
A <- ggplot(df, aes(x = representativeness, fill = status, group = status)) +
  geom_density(alpha = 0.5) + 
  scale_fill_brewer(palette = "Set2") +
  ggtitle("Diseased vs Not Diseased") + 
  theme(legend.position = "bottom") +
...
B <- ggplot(df_tested, aes(x = representativeness, fill = obs_status, group = obs_status)) +
  geom_density(alpha = 0.5) + 
  scale_fill_brewer(palette = "Set3")+
  ggtitle("Diagnosed vs Not Diagnosed") +
  theme(legend.position = "bottom")  +
