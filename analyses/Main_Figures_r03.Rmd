...
b <- expand.grid(R, c(0,1)) %>%
  ...
  mutate(phi = diagnosis_fn(representativeness = R, x = X, theta = c(-15, 20, 3))) %>%
...
df_tested <- test_cross_sectional(df, theta = c(-15, 20, 3))
```
...
```{r}
eg_diagnosed <- test_longitudinal(eg_long, theta = c(-15, 20, 5))
