...
```{r}
df <- generate_cs_negatives(prevalence = c(0.15, 0.075), n = 100000)
...
b <- expand.grid(R, c(0,1)) %>%
  ...
  mutate(phi = diagnosis_fn(severity = R, x = X, theta = c(-12, 20, -3))) %>%
  ...
  ylim(c(0,1)) +
  coord_fixed() +
  scale_color_brewer(palette = "Set1", direction = -1, labels = c("X = 0", "X = 1")) +
  ylab("P(Follow up | R)") +
...
```{r}
df <- generate_cs_negatives(n = N, prevalence = 0.1)
```
...
c <- ggplot(df, aes(x = severity, fill = disease, group = disease, color = disease, alpha = disease)) +
  geom_density() + 
...
d <- ggplot(df_tested, aes(x = severity, fill = diagnosed, group = diagnosed, color = diagnosed, alpha = diagnosed)) +
  geom_density() + 
...
```{r}
df <- generate_cs_negatives(N, prevalence = 0.1)
...
c <- ggplot(df_tested, aes(x = severity, fill = status, group = status, color = status, alpha = status)) +
  geom_density() + 
...
d <- ggplot(df_tested, aes(x = severity, fill = obs_status, group = obs_status, color = obs_status, alpha = obs_status)) +
  geom_density() + 
...
```{r}
df <- generate_cs_negatives(prevalence = c(0.15, 0.075), n = N)
```
