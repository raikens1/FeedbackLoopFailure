...
In Case 2A, $\beta_0 = -13$, $\beta_1 = 20$, and $\beta_2 = -3$. The constant, $c_{x_i} = \frac{1}{1 + exp(-(\beta_0 + \beta_2X_i))}$ is again a corrective constant to ensure that $P(\text{evaluated}| R_i(t), X_i) = 0$.  Note that $c_{x_i}$ depends on $\beta_0$, $\beta_1$, $\beta_2$, and $X_i$.
