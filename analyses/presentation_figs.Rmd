...
# Case 0: Diagnosis depends on Severity only
...
ggplot(plt_data, aes(x = severity, group = group, alpha = group)) +
...
# Case 1: Diagnosis depends on severity and demographics
...
ggplot(plt_data, aes(x = severity, group = group, alpha = group, fill = x)) +
  geom_histogram(position = "identity") +
...
ggplot(sample_docs, aes(x = S, y = p_diagnose, group = doctor_id, color = as.factor(X))) +
  geom_line() + 
  facet_wrap(~X) +
  scale_color_brewer(palette = "Set1", direction = -1: E) +
  ylab("P(Diagnosis | S)")
```
