...
x.labs = c("FALSE" = "Demographic characteristic absent", "TRUE" = "X = 1")
...
df <- generate_cs_beta(n = N) %>%
  mutate(status = case_when(!disease ~ "No disease",
                            !x ~ "Disease, Demographic characteristic absent",
                            x ~ "Disease, X = 1")) %>%
  mutate(status = factor(status, levels = c("No disease", "Disease, Demographic characteristic absent", "Disease, X = 1"))) %>%
  mutate(status_case1 = ifelse(!disease, "No disease", "Disease, both Demographic characteristic absent and X = 1")) %>%
  mutate(status_case1 = factor(status_case1,
                               levels = c("No disease", "Disease, both and X = 1", "Disease, Demographic characteristic absent", "Disease, X = 1")))
...
case2a_hist <- ggplot(df, aes(x = representativeness, fill = status, group = status, color = status, alpha = status)) +
  ...
  geom_histogram(dat = filter(df, status == "Disease, demographic characteristic absent"), aes(x = representativeness, y = stat(count)* 10), position = "identity") +
...
a <- data.frame(R = R, phi = diagnosis_fn(R, 0, theta = c(-12, 20, 0))) %>%
  ...
  mutate(X = factor(X, levels = c("Both X = 0 and X = 1", "Demographic characteristic absent", "X = 1"))) %>%
...
b <- expand.grid(R, c(0,1)) %>%
  ...
  ylim(c(0,1)) +
  coord_fixed() +
  scale_color_brewer(palette = "Set1", labels = c("demographic characteristic absent", "X = 1")) +
  ylab("P(Evaluated | R, X)") +
...
plt_data <- rbind(binom.confint(sum(df_x1$diagnose), dim(df_x1)[1], method = "asymptotic"), 
  ...
  mutate(x.status = c("X = 1, Diagnosed", "Demographic characteristic absent, diagnosed", "X = 1, Disease", "Demographic characteristic absent, Disease"))
...
case2a_incidence <- ggplot(plt_data, aes(x = status, y = Incidence, fill = x.status, group = x, alpha = x.status)) +
  ...
  scale_fill_manual(name = "", values = c(x1_color, x1_color, x0_color, x0_color)) +
  ylim(c(0,0.175))
```
