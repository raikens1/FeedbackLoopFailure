...
case1_hist <- ggplot(df, aes(x = representativeness, y = stat(count), fill = disease, group = disease, color = disease, alpha = disease)) +
...
```{r figure2 with histograms, fig.width=8, fig.height=7}
p <- ggarrange(ggarrange(case1_hist, case2a_hist, case2b_hist, case3_hist,
                         ncol = 2, nrow = 2, legend = "none", labels = "AUTO"),
          ggarrange(a, b, c, ncol = 3, nrow = 1, common.legend = T,
                    legend = "bottom", labels = c("E", "F", "G")), 
          nrow = 2, ncol = 1, heights = c(3,2))

p

ggsave("figures/setup_histogram.pdf", p,  width=9, height=7)
```
...
case2a_incidence <- ggplot(plt_data, aes(x = status, y = Incidence, fill = x, group = x, alpha = status)) +
...
#ggarrange(case2a_incidence, case2a_rep, ncol = 2, widths =c(3,7), labels = "AUTO")
```
...
# version using iris as a standin for table 2
ggarrange(ggarrange(case2a_incidence, case2a_rep, case2b_incidence, case2b_rep,
          ncol = 2, nrow = 2, widths =c(2,3), legend = F, labels = "AUTO"), 
          ggarrange(ggtexttable(head(iris), theme = ttheme("light")), labels = "E"),
          ncol = 1, nrow = 2, heights = c(2, 1))
```

```{r fig.height=6, fig.width=8}
# version without table appended
ggarrange(case2a_incidence, case2a_rep, case2b_incidence, case2b_rep,
          ncol = 2, nrow = 2, widths =c(2,3), legend = F, labels = "AUTO")

# version without table appended
p <- ggarrange(case2a_incidence, case2a_rep, case2b_incidence, case2b_rep,
          ncol = 2, nrow = 2, widths =c(2,3), legend = F, labels = "AUTO")
...
p <- ggarrange(a, b, ncol = 2, widths =c(2,3), legend = F, labels = "AUTO")
ggsave("figures/case3.pdf", p, width = 8, height = 3)
```

## Case 4

Finally, we'll consider a situation in which the diagnostic evaluation itself has some error rate.  Suppose the test has a sensitivity and specificity of 95%.
